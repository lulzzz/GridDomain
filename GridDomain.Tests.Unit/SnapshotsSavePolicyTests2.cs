...
{
    public class SnapshotsSavePolicyTests2
    {
        [Fact]
        ...
        [Fact]
        public async Task Given_policy_with_frequency_limitations_When_saved_too_frequently_and_persist_take_too_much_time_Then_should_save_after_time_passed()
        {
            var policy = new SnapshotsPersistencePolicy(1,10,TimeSpan.FromSeconds(1));
            Assert.True(policy.ShouldSave(1));
